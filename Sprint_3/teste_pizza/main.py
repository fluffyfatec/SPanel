...
df_totais = pd.merge(row,df_vacinastratado, how='inner', on='nome_munic')
...
df_tratado_rename = df_totais.rename(
                columns={'nome_munic': 'Município', 'casos': 'Casos Acumulados', 'datahora': 'Data da Atualização','casos_novos': 'Casos Novos',
                         'obitos': "Óbitos Acumulados", "obitos_novos": "Óbitos Novos","pop": "População","doseunica":"Dose Unica","primeiradose":"Primeira Dose",
                         "segundadose":"Segunda Dose","terceiradose":"Terceira Dose"})
df_tratado_rename = pd.DataFrame(df_tratado_rename,
                columns=['Município', 'Casos Acumulados', 'Casos Novos', 'Óbitos Acumulados', 'Óbitos Novos','Dose Unica','Primeira Dose','Segunda Dose','Terceira Dose'
                    ,'População', 'Data da Atualização'])
                    ...
                    dbc.CardBody([
                         dcc.Dropdown(
                            ...
                            dt.DataTable(
                                ...
                                'minWidth': '100px', 'width': '95px', 'maxWidth': '95px','height': '200px'}),
                                editable=False,
                                sort_action="native",
                                sort_mode="single",
                                row_deletable=False,
                                ...
                                style_table={'overflowY': 'auto','height': '15px','overflowX': 'scroll','height': '250px'},
...
def update_table(location):
    ...
    if location is not None:
        df_municipios = df_tratado_rename[df_tratado_rename["Município"].isin(location)]
