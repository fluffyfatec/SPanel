...
fig3 = go.Figure()
fig3.add_trace(go.Pie(values= df_vacinas["Total Doses Aplicadas"], labels=df_vacinas["Dose"]))
fig3.update_layout(title_text='Imunização')
...
# ==================================================================
    ...
    , dbc.Row([
        dbc.Col([
            dcc.Graph(id="vacinas-graph", figure=fig3) #COLOCA AQUI OS GRAFICOS IMUNIZADOS LUIZ
        ], md=6)
        , dbc.Col([
...
def display_vacinas(location):
    df_vacinas = pd.read_csv('docs/vacinas', sep=";")
    if not location:
        df_vacinas = df_vacinas[df_vacinas["Município"] == location]
        fig3.update_traces(go.Pie(labels=df_vacinas["Dose"], values=df_vacinas["Total Doses Aplicadas"], autopct="%1.2f%%"))
    else:
        df_vacinas = df_vacinas[df_vacinas["Município"] == location]
        fig3.update_traces(go.Pie(labels=df_vacinas["Dose"], values= df_vacinas["Total Doses Aplicadas"], autopct="%1.2f%%"))
    return (fig3)
